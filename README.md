## Hi there 👋 Mi nombre es Jesus Eduardo Ambriz Olavarria

I'm a Systems Engineer, Java Jr Developer, and tech mentor in Alfresco and Liferay with a passion for impelemnt Open Sorce Software.
I pass over all TI Systems Areas:
- Support
- Develop
- Infraestructure
- Arquitecture
- Tech Leader

👨‍🏫 Current Focus
Tech lead and Proyect Manager 

## 💼 Experiencia y Competencias Técnicas

### 🌐 Portales Empresariales
- **Liferay Portal / DXP** (6.x–7.4)
  - Implementaciones **On-Premise** y **Cloud**
  - Desarrollo de **Portlets**
  - Desarrollo de **Temas (Themes)**
  - Desarrollo de **Microportlets**

### 📂 ECM / Gestión Documental
- **Alfresco ECM** (Community Edition / Enterprise Edition)
- **EMC Documentum**
- Integración mediante **CMIS (Content Management Interoperability Services)**

### 👥 CRM
- **SugarCRM**
- **VtigerCRM**
- **Salesforce**
- Desarrollo de módulos personalizados
- Integración con sistemas externos
- Migración de datos
  
### ☁️ Arquitectura y Nube
- Arquitectura de Software
- Enterprise Architecture
- Google Cloud Platform (GCP)
- Microsoft Azure
- Red Hat OpenShift
- Arquitectura basada en Microservicios
  - Spring Boot
  - Vue.js

### 🔗 Integración de Sistemas
- Desarrollo e integración de servicios **REST**
- Desarrollo e integración de servicios **SOAP**
- Análisis funcional
- Migración de servicios **SOAP → REST**

### 🗄️ Gobernanza de Datos
- Diseño de modelos de datos mediante **UML**
- Estándares de Arquitectura Empresarial
- Gobierno de Datos conforme a lineamientos de la **CNBV**

### 👨‍💼 Liderazgo Técnico
- Gestión de equipos de desarrollo
- Administración de PMO
- Control de calidad de entregables
- Elaboración de documentación técnica
- Capacitación técnica
- Instructor Certificado en **Liferay**

### 🚀 DevOps y Control de Versiones
- Git
- Jenkins
- SVN
- Docker
- Kubernetes *(nivel básico)*

### 🛢️ Bases de Datos
- Oracle
- MySQL
- PostgreSQL
- Microsoft SQL Server

### 📋 Metodologías y Gestión
- Análisis de requerimientos
- UML
- Gestión de proyectos
- Mejora continua de procesos

<!--
**jeaolucas/jeaolucas** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...

- 🌱 I’m currently learning ...

- 👯 I’m looking to collaborate on ...

- 🤔 I’m looking for help with ...

- 💬 Ask me about ...

- 📫 How to reach me: ...

- 😄 Pronouns: ...

- ⚡ Fun fact: ...

-->
